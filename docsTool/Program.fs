...
module ProjInfo =
    ...
    let findTargetPath targetPath =
        if File.exists targetPath then
            ...
            let debugFolder =
                sprintf "%cDebug%c" Path.DirectorySeparatorChar Path.DirectorySeparatorChar

            let releaseFolder =
                sprintf "%cRelease%c" Path.DirectorySeparatorChar Path.DirectorySeparatorChar

            let debugFolderAlt =
                sprintf "%cDebug%c" Path.DirectorySeparatorChar Path.AltDirectorySeparatorChar

            let releaseFolderAlt =
                sprintf "%cRelease%c" Path.DirectorySeparatorChar Path.AltDirectorySeparatorChar

            let releasePath =
                targetPath
                    .Replace(debugFolder, releaseFolder)
            ...
            then
                ...
                failwithf
                    "Couldn't find a dll to generate documentationfrom %s or %s"
                    targetPath
                    releasePath
    ...
    let findReferences projPath : ProjInfo =
        ...
        | Ok options ->
            ...
            let references =
                ...
                |> Array.filter (fun s -> s.StartsWith(RefPrefix))
                |> Array.map (fun s ->
        ...
        | Error e -> failwithf "Couldn't read project %s - %A" projPath e
...
module GenerateDocs =
    ...
    let generateDocs
        (libDirs: ProjInfo.References)
        (docSourcePaths: IGlobbingPattern)
        (cfg: Configuration)
        =
            ...
            let doc =
                ...
                let compilerOptions = [|
                    ...
                    yield!
                        rref
                        |> Seq.filter (
                            stringContainsInsenstive "fsharp.core.dll"
                            >> not
                        )
                        |> Seq.filter (
                            stringContainsInsenstive "NETStandard.Library.Ref"
                            >> not
                        ) // --targetprofile:netstandard will find the "BCL" libraries
                |]
    ...
    let buildDocs (projInfos: ProjInfo.ProjInfo array) (cfg: Configuration) =
        let refs = [|
            yield!
                projInfos
                |> Array.collect (fun p -> p.References)
                |> Array.distinct
            yield!
                projInfos
                |> Array.map (fun p -> p.TargetPath)
        |]

        copyAssets cfg

        let generateDocs =
            async { return generateDocs refs (docsFileGlob cfg.DocsSourceDirectory.FullName) cfg }
    ...
    let renderDocs (cfg: Configuration) =
        let projInfos =
            ...
            |> Seq.map (ProjInfo.findReferences)
            |> Seq.toArray
...
module WebServer =
    ...
    let createWebsocketForLiveReload (httpContext: HttpContext) (next: unit -> Async<unit>) =
        async {
...
let main argv =
    ...
    use __ =
        AppDomain.CurrentDomain.ProcessExit.Subscribe(fun _ -> dispose tempDocsOutDir)

    use __ = Console.CancelKeyPress.Subscribe(fun _ -> dispose tempDocsOutDir)
