...
let signature (m: Member) =
    seq {
        if
            ...
            |> not
        then
            yield code [ Class "function-or-value" ] [ str m.Details.Signature ]
    }

let repoSourceLink (m: Member) =
    seq {
        if
            m.Details.FormatSourceLocation
            |> String.IsNullOrEmpty
            |> not
        then
            yield
...
let commentBlock (c: Comment) =
    ...
    let renderSection (s: KeyValuePair<string, string>) : Fable.React.ReactElement list =
        ...
        | NonEmptyDefaultBlock content -> [
            div [ Class "comment-block" ] [ RawText(normalize content) ]
          ]
            ...
            table [ Class "table" ] [
                ...
                tbody [] [
                    for it in members do
                        ...
                        yield
                            tr [] [
                                td [] [ Helpers.createAnchorIcon (it.Details.FormatUsage(40)) ]
                                ...
                                td [ Class "member-name" ] [ yield! signature it ]
